...
export class JournalEntryModal extends Modal {
	...
	onOpen() {
		...
		const entryTabContent = new JournalEntryTab(
			contentContainerEl,
			this.plugin,
			{ clearButton, insertButton },
			this
		);
		...
		journalStructuresTab.addEventListener('click', () =>
			this.switchTab('journal-structures', journalStructuresTab, allNavItems)
		);
