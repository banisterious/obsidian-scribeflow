...
export class TemplateWizardModal extends Modal {
	...
	private renderHeader(): void {
		...
		const subtitleText = this.isEditMode
			? 'Modify your existing journal template'
			: 'Create a new journal template for ScribeFlow';
	...
	private renderTemplateSelection(container: HTMLElement): void {
		...
		const pluginDisplayName =
			this.templatePluginSource === 'templater' ? 'Templater Plugin' : 'Core Templates Plugin';
		...
		new Setting(templateSection)
			...
			.addDropdown(dropdown => {
				dropdown.addOption(
					'',
					`Select ${this.templatePluginSource === 'templater' ? 'Templater' : 'Core Templates'} template...`
				);
	...
	private renderTemplateImportContent(container: HTMLElement): void {
		...
		new Setting(container)
			...
			.addTextArea(textarea => {
				textarea.setValue(this.templateContent).onChange(value => {
					this.templateContent = value;
					this.renderNavigation(); // Update button state
				});
	...
	private renderPredefinedStructureContent(container: HTMLElement): void {
		container.createEl('p', {
			text: 'Select a predefined structure to use as a starting point:',
			cls: 'sfp-step-description',
		});
...
>>> {{metrics}}`,
	...
	private renderNavigation(): void {
		...
		if (this.currentStep > 1 && !(this.isEditMode && this.currentStep === 2)) {
			new ButtonComponent(leftButtons).setButtonText('Back').onClick(() => {
				this.currentStep--;
				this.renderCurrentStep();
			});
		}
		...
		// Cancel button
		new ButtonComponent(leftButtons).setButtonText('Cancel').onClick(() => this.close());
		...
		const placeholderGroups = [
			...
			},
			{
				...
				options: [
					...
					{
						value: '{{metrics-inline}}',
						label: '{{metrics-inline}} - Comma-separated (includes word count)',
					},
				],
			},
		];
